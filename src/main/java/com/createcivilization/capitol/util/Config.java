...
public class Config {
	...
	public static void loadConfig() throws IOException {
		...
		FileUtils.setContentsIfEmpty(file, "{\"claimRadius\": 1, \"debugLogs\": false, \"inviteTimeout\": 60}");
	...
	public static void saveConfig() throws IOException {
		...
		writer.name("debugLogs").value(debugLogs.get());
		writer.name("inviteTimeout").value(inviteTimeout.get());
		writer.endObject();
		writer.close();
