...
public abstract class ChunkDataImpl implements IChunkData {
	...
	public void updateTakeOverProgress(MinecraftServer server) {
		...
		OldTeam oldTeam = TeamUtils.getTeam(pos, dimension).getOrThrow();
		...
		for (War loadedWar : DataManager.WarData.loadedWars) {
			boolean isDeclaring = loadedWar.getDeclaringTeam().getTeamId().equals(oldTeam.getTeamId());
		...
		if (this.getTakeOverProgress() >= maxTakeOver) {
			...
			this.resetTakeOverProgress();
			NeoForge.EVENT_BUS.post(new WarEvent.ChunkTakenOverEvent(self, oldTeam));
			LogToDiscord.postIfAllowed(
				oldTeam,
				"Chunk taken from oldTeam " + oldTeam.getName() +" over in war , at ChunkPos " + pos
			);
		}
