...
public class ActivateWarTeamsDebugCommand extends AbstractDebugCommand {
	...
	public int executeAllParams(CommandContext<CommandSourceStack> context) {
		...
		OldTeam attackerOldTeam = TeamUtils.getTeamByName(attackerTeamName).getOrThrow();
		OldTeam defenderOldTeam = TeamUtils.getTeamByName(defenderTeamName).getOrThrow();
		DataManager.WarData.loadedWars.add(new War(attackerOldTeam, defenderOldTeam));
