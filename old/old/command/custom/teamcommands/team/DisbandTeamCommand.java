...
public class DisbandTeamCommand extends AbstractTeamCommand {
	...
	public int execute(Player player) {
		// Only runs if player owns the receivingOldTeam (check canExecute)
		TeamUtils.removeTeam(TeamUtils.getTeam(player).getOrThrow().getTeamId());
		player.sendSystemMessage(Component.literal("OldTeam successfully disbanded"));
		return 1;
	}

	@Override
	public boolean canExecute(Player player) {
		setMustWhat("be a player and own a receivingOldTeam");
