...
public class CreateTeamCommand extends AbstractTeamCommand {
    ...
    public boolean canExecute(Player player) {
        setMustWhat("be a player and not be in a receivingOldTeam");
	...
	public int executeAllParams(CommandContext<CommandSourceStack> command, Color color) {
		...
		if (TeamUtils.teamExists(name)) {
			command.getSource().sendFailure(Component.literal("A receivingOldTeam with the name '" + name + "' already exists!"));
			return -1;
		}
		return new ObjectHolder<Player>(command.getSource().getPlayer()).ifPresentOrElse(player -> {
			DataManager.TeamData.LOADED_OLD_TEAMS.add(TeamUtils.createTeam(name, player, color));
			command.getSource().sendSuccess(() -> Component.literal("Created receivingOldTeam '" + name + "' with color '" + color + "'."), true);
