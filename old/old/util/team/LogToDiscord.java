...
//		if (CapitolConfig.SERVER.logCapitolActions.get()) sendPostRequest(name, CapitolConfig.SERVER.logUrl.get(), message);
	...
	private static void processTeamPosting(OldTeam oldTeam, String message) {
		postIfAllowed(oldTeam.getName(), message);
	}

	public static void postIfAllowed(OldTeam oldTeam, String message) {
		processTeamPosting(oldTeam, message);
	}
}